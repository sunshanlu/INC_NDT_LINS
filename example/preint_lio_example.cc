...
IncNdt::Options ndt_options = {1.0, 1.0, 100000, 5.0, IncNdt::NearbyType::CENTER};    ///< 增量ndt配置项
...
int main(int argc, char **argv) {
    ...
    PreinteLio::Options lio_options = {true, SE3d(Ril, til).inverse(), 0.3, 3 * M_PI / 180.0, 0.1, 2 * M_PI / 180.0};
    ...
    io.SetPointCloudCallback([&](FullPointCloud::Ptr cloud, double stamp) { preint_lio->AddCloud(cloud, stamp); },
        ...
        .SetIMUCallback([&](IMU::Ptr imu) { preint_lio->AddIMU(imu); }, "/imu/data")
        .Go();
    return 0;
}
